...
(function () {
  ...
  Chrono.lab.register({
    ...
    aside: () => CL.mode === "light" ? `
      ...
      <p>Now move the clock sideways. Seen by us, the light must travel a longer, <b>diagonal</b> path — but light always travels at the same speed. So each tick takes longer. The moving clock <i>really</i> ticks slower, by a factor physicists call gamma: ${CL.v >= 0.95 ? `at ${Math.round(CL.v * 100)}% of light speed it's ${(1 / Math.sqrt(1 - CL.v * CL.v)).toFixed(1)}, so in your frame one year on the moving clock takes about ${Math.round(1 / Math.sqrt(1 - CL.v * CL.v))} of yours` : "at 87% of light speed it's about 2, so in your frame the moving clock ticks about half as fast"}.</p><p class="meta">For the technically minded: γ = 1/√(1 − v²/c²).</p>
