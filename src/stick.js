...
(function () {
  ...
  const PIC = {
    ...
    stars: ["An onion built from the inside out: each new layer is the ash of the fire before, burned again at a higher heat.",
      "Real stars mix, and their layers aren't sharp. And each new fire only lights because the core squeezes itself hotter first."],
  ...
  const TRAP = {
    ...
    stars: ["The Sun will one day make gold.", "Stars are famous for forging the elements, and for decades the textbook answer was that exploding stars made the gold.",
      "No star makes gold by burning: fusion stops at iron, even in the biggest stars. Gold is built another way, by nuclei soaking up neutrons: a little slowly, inside ageing giant stars, and most in violent events, above all colliding neutron stars (seen in 2017). The Sun is too small for either. <span class=\"tag ESTABLISHED\">Established</span> How the gold splits between colliding neutron stars and rare kinds of collapsing star is still argued over.", "CONTESTED"],
  ...
  const QB = {
    ...
    films: { q: "In Two Films, why do two universes with identical starting frames end up different?", o: ["The simulation uses random numbers", "With two time directions, even a complete snapshot of 'now' doesn't fix the future", "Rounding errors in the computer"], a: 1,
      why: "Both films are exact solutions of the same equation, identical at the starting frame. With two times, complete starting data doesn't determine what this equation does next." },
    ...
    timeline: { q: "On the app's powers-of-ten timeline, from the Planck time to the last black hole, where is today?", o: ["Near the very end", "A little under halfway along", "At the very start"], a: 1,
      why: "About 40% across the chart: as many powers of ten lie between the Planck time and today as between today and the last black holes. That's a place on a chart with chosen endpoints, not a fraction of the universe's lifetime." },
    stars: { q: "Why does fusion in a big star's core stop at iron?", o: ["The star runs out of hydrogen", "Fusing iron takes energy instead of giving it", "Iron sinks to the centre where it's too cold"], a: 1,
      why: "Up to iron, fusion gives energy out, which holds the star up. Iron can't: the core collapses in under a second and the star explodes." },
