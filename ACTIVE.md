...
**3c done** (control hints in Learn mode, mission circles, Try boxes wait for missions; explanation length deferred by Will). **Phase 3 complete** (3d: Flatland on the template; phone pass with a new `35-phone` regression check; Atlas list on phones). **Phase 4 in progress:** 4a Passport, 4b challenge links, 4c teacher mode done. **Next:** 4d regression additions (locked predictions show no answer numbers; stop setups in range; no shared prediction keys). Stars lab (`src/stars.js`, D-048) fitted in between.
