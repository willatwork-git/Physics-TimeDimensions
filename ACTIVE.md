...
universe won't end as one black hole (D-052). New labs follow **the lab pattern** in `DESIGN.md`. Assets at `?v=94`. `tools/regress.sh`: 16 pass, 1 warning (below).
...
**Waiting on Will:** try the reworked Stars lab; read its aside and element stories (`src/stars.js`) and the new "Stars" section of the arrow-of-time concept (`src/concepts.js`) and the two new Black holes
