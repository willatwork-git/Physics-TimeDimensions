...
window.addEventListener("load", () => setTimeout(async () => {
  ...
  for (const [id, M] of Object.entries(Chrono.MATHS)) {
    Chrono.progress.setPred(id, { guess: -1 }); location.hash = "#" + id; await W(250);
    ...
    for (const x of M.examples) {
      out.n++;
      try { d.applySetup(x.setup); const got = x.read(d.state());
        if (!(Math.abs(got - x.expect) <= x.tol)) out.bad.push(`${id} · ${x.what}: the lab gives ${got}, the page says ${x.expect} (± ${x.tol}; ${x.from})`); }
    ...
    if (rows < M.examples.length) out.bad.push(`#maths/${id}: page shows ${rows} table rows, expected at least ${M.examples.length}`);
  }
