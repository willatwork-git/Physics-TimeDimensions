...
(function () {
  ...
  Chrono.MISSIONS = {
    ...
    stars: [
      { id: "iron", title: "Find the smallest star whose core makes iron",
        hint: "One star: raise Star mass from our Sun and play each life. Watch which tiles its core lights up.",
        check: s => s.mode === "star" && s.mass === 12 && s.stage >= 5,
        near: s => s.mode === "star" ? Math.min(1, s.mass / 12) * (s.mass === 12 ? 0.5 + 0.5 * s.stage / 5 : 0.8) : 0, hold: 800,
        reveal: "About ten times the Sun's mass (12 is the first size on this slider that gets there). Smaller stars stop at oxygen, or at neon and magnesium, and end as white dwarfs. Our Sun will never make iron: every iron atom in your blood came from a bigger star, or a white dwarf that exploded.",
        tags: ["ESTABLISHED", "CONTESTED"], show: { mode: "star", star: 5, p: 5.1, play: true } },
      { id: "dwarf", title: "Play our Sun's whole life — what's left at the end?",
        hint: "One star: set Star mass to 1 Sun and press Play its life. Let it run to the end.",
        check: s => s.mode === "star" && s.mass === 1 && s.ended,
        near: s => s.mode === "star" && s.mass === 1 ? 0.3 + 0.7 * Math.min(1, s.stage / 2) : 0, hold: 400,
        reveal: "A white dwarf: a hot core the size of Earth, made of the carbon and oxygen the Sun built, slowly cooling. The outer layers drift off into space, carrying some carbon and nitrogen for the next stars. That's about five billion years from now.",
        tags: ["ESTABLISHED"], show: { mode: "star", star: 2, p: 1.9, play: true } },
      { id: "gold", title: "Find the earliest time there was any gold",
        hint: "Generations: tap gold (Au), then step Cosmic time back until its tile goes dark.",
        check: s => s.mode === "history" && s.step === 2,
        near: s => s.mode === "history" ? 1 - Math.min(1, Math.abs(s.step - 2) / 3) : 0, hold: 800,
        ...
        tags: ["CONTESTED"], show: { mode: "history", step: 2, sel: 79 } },
      { id: "sunforms", title: "Go to the moment the Sun and Earth formed",
        hint: "Generations: step Cosmic time forward to when the Sun forms.",
        check: s => s.mode === "history" && s.step === 4,
        near: s => s.mode === "history" ? 1 - Math.min(1, Math.abs(s.step - 4) / 4) : 0, hold: 800,
        reveal: "9.2 billion years after the Big Bang. A 25-Sun star lives about 7.5 million years, so by then one could have lived and died more than a thousand times over, each generation seeding the gas for the next. The Sun and Earth were built from what those short, bright lives made. Stars a little smaller than the Sun from the early generations are still shining: among the oldest stars we can see.",
        tags: ["ESTABLISHED"], show: { mode: "history", step: 4 } }
    ],
    energy: [
