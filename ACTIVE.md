...
and wired (D-048): 21 labs, 41 missions. The Black holes lab now says what evaporation does to entropy and why the
universe won't end as one black hole (D-052). The element table has an animated atom and a Star size slider (the Sun stops at oxygen; iron needs ~10 Suns). Assets at `?v=91`. `tools/regress.sh`: 16 pass, 1 warning (below).
...
**Waiting on Will:** read the Stars element stories and aside (`src/stars.js`, `STORY`) and the two new Black holes
