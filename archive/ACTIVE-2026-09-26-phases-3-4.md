...
**3c done** (control hints in Learn mode, mission circles, Try boxes wait for missions; explanation length deferred by Will). **Phase 3 complete** (3d: Flatland on the template; phone pass with a new `35-phone` regression check; Atlas list on phones). **Phase 4 complete** (Passport, challenge links, teacher mode + worksheets, tour-stop regression). Only the guess counter remains — it needs a server, so it waits for the domain move. Stars lab (`src/stars.js`, D-048) fitted in between.
