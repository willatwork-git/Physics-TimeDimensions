...
(function () {
  ...
  const PAGES = {
    ...
    arrow: {
      ...
      sections: [
        ...
        ["Stars: where the ordered start gets used", `<p>A smooth early universe had one big store of order left: gas that gravity could gather. Gravity pulls it into stars, and a star is an engine. It fuses light elements into heavier ones — the carbon, oxygen and iron that planets and people are made of — and sends the energy out as sunlight. When it dies it scatters what it made, and the next stars and planets form from that. <span class="tag ESTABLISHED">Established</span></p>
          <p>Earth then takes in that sunlight and sends the same energy back out as about twenty times as many infrared photons. That trade — few, energetic photons in; many, cooler ones out — is the low entropy everything here runs on. See <a href="#stars">Stars forge the elements</a> and <a href="#energy">Earth's energy budget</a>.</p>`],
        ["Entropy climbs; complexity rises and falls", `<p>Low entropy isn't calm — it means plenty of <b>free energy</b> is still unspent: potential that can still make things happen. The early universe was a hot, fast-moving soup, but spread almost perfectly evenly, and for gravity evenness is the unspent state. Everything since has been gravity spending it. See <a href="#concepts/entropy">Is entropy a thing?</a> <span class="tag ESTABLISHED">Established</span></p>
          <p>Think of a river. <b>Entropy</b> is how far downstream you are: it only increases. <b>Activity</b> is how fast the water is dropping: the rapids. The universe's rapids are behind it — stars were born fastest about 10 billion years ago, "cosmic noon", and today at about a tenth of that rate. Ahead: small red dwarfs burning quietly for trillions of years, then embers. <span class="tag ESTABLISHED">Established</span> (the first stars being huge is predicted, not yet seen <span class="tag CONTESTED">Contested</span>)</p>
          <p>And <b>complexity lives in the middle</b>. Pour cream into coffee: at first it sits on top — simple. At the end it's evenly mixed — simple again. In between come swirls and tendrils. Entropy rises throughout; complexity rises, then falls. Stars, planets and people are eddies in that current, existing only while energy flows downhill. <span class="tag CONTESTED">Contested</span> (the picture is widely used; how to measure complexity isn't settled)</p>
          <p>See it: the complexity line in the <a href="#entropy">Entropy box</a>, the star-birth curve on the <a href="#timeline">Cosmic timeline</a> (ordinary time), stars as engines in <a href="#stars">Stars forge the elements</a>, and Earth living off the flow in <a href="#energy">Earth's energy budget</a>.</p>
          <p><b>At the edge.</b> Some physicists suspect gravity itself is a large-scale, statistical effect, like temperature — see <a href="#atlas/jacobson">Gravity as thermodynamics</a> <span class="tag CONTESTED">Contested</span>. And some argue the lumpiness of the universe distorts how we read its expansion, perhaps enough to explain dark energy — see <a href="#atlas/timescape">Timescape: clocks in voids</a> <span class="tag CONTESTED">Contested</span>.</p>`],
    ...
    entropy: {
      ...
      sections: [
        ...
        ["Is entropy a thing?", `<p>Not the way gravity is. <b>Entropy is a count</b>, not a force or a substance: how many arrangements look the same from outside. Nothing pushes a gas to spread; there are just overwhelmingly more spread-out arrangements for it to wander into. The second law is odds, not a push. <span class="tag ESTABLISHED">Established</span></p>
          <p>A still gas has entropy, but entropy only <i>means</i> something for time when states change: the arrow needs a before and an after to compare. A metre is still a metre if time stops; the arrow of time isn't.</p>
          <p>Keep a second word for <b>potential</b>. Physics calls it <b>free energy</b>: the part of a system's energy still available to do work — its energy minus its temperature times its entropy. As entropy rises, free energy falls. A stretched rubber band pulls back because letting go frees its long molecules into far more arrangements; the pull is a <i>free-energy</i> gradient, and physicists call it an <b>entropic force</b>. Two words, two jobs: free energy is what can still happen; entropy counts how spread out things already are. <span class="tag ESTABLISHED">Established</span></p>
          <p>That's also why "gravity from entropy" isn't absurd: if rubber bands can pull for statistical reasons, perhaps gravity does too. It's unproven — see <a href="#atlas/jacobson">Gravity as thermodynamics</a>. <span class="tag CONTESTED">Contested</span></p>`],
    ...
    radiation: {
      title: "Radiation: the beginning and the end", icon: "✺", tags: ["ESTABLISHED", "CONTESTED"],
      short: "The universe began as radiation, is rich in matter in its middle, and ends as radiation again once the black holes evaporate. Everything interesting happens in between.",
      one: "Radiation, then matter, then radiation again — with the universe's free energy spent along the way, never made anew.",
      sections: [
        ["What radiation is", `<p>Light is radiation: packets of energy called photons. Every warm thing glows — you in infrared, the Sun mostly in visible light — and the colour of the glow gives the temperature. Explaining that glow is where quantum physics began (Planck, 1900). Photons carry entropy as well as energy: many cool photons hold more than a few hot ones with the same energy. <span class="tag ESTABLISHED">Established</span></p>`],
        ["The beginning: a universe of light", `<p>For its first ~50,000 years the universe's energy was mostly radiation; then matter took over. When it had cooled enough for atoms to form, 380,000 years in, that light was set free. It's still here as the microwave background, stretched from about 3,000 degrees to 2.7 above absolute zero, with about a billion photons for every atom. <span class="tag ESTABLISHED">Established</span></p>`],
        ["The middle: matter's time", `<p>Gravity spends the free energy stored in the evenness: gas clumps, stars ignite and forge the elements, planets and life form in the flow of starlight. Earth takes in sunlight and sends the same energy out as about twenty times as many infrared photons — and runs on the difference. This is where complexity lives. <span class="tag ESTABLISHED">Established</span></p>`],
        ["The end: light again", `<p>Stars burn out; over 10¹⁹–10²⁰ years close passes fling most of them out of their galaxies, and the rest slowly spiral inward. Black holes hold the most entropy anything can, but not for ever: they evaporate as Hawking radiation, from 10⁶⁷ years for one the Sun's mass to around 10¹⁰⁰ for the largest. What's left is a thin, ever-colder bath of radiation — "heat death" means <i>even</i>, not empty: nothing left to flow downhill. <span class="tag ESTABLISHED">Established</span> (the processes) · <span class="tag CONTESTED">Contested</span> (whether protons and dark matter last for ever)</p>`],
        ["Where the potential goes", `<p>It isn't made again. Free energy is spent at every step — clumping, starlight, black holes, evaporation — and entropy counts the spending. Meanwhile expansion stretches radiation: each photon's wavelength grows and its energy falls. Across an expanding universe, total energy isn't conserved at all — the laws aren't the same from moment to moment, which is what energy conservation needs. In any lab, energy is always conserved. <span class="tag ESTABLISHED">Established</span> (some physicists prefer to say the energy goes into the gravitational field — a matter of bookkeeping they still argue about <span class="tag CONTESTED">Contested</span>)</p>`],
        ["Other kinds of radiation", `<p>Gravity radiates too: merging black holes send out <b>gravitational waves</b>, ripples of spacetime itself, first detected in 2015. And a background of neutrinos from the first seconds fills space, not yet seen directly. <span class="tag ESTABLISHED">Established</span> (gravitational waves) · the neutrino background is inferred, not detected</p>`]
      ],
      labs: [["#timeline", "Cosmic timeline"], ["#hawking", "Black holes evaporate"], ["#energy", "Earth's energy budget"], ["#stars", "Stars forge the elements"], ["#horizons", "Cosmic horizons"], ["#concepts/arrow", "The arrow of time"]],
      sources: "M. Planck (1900); A. Penzias & R. Wilson, ApJ 142, 419 (1965); Planck Collaboration, A&A 641, A6 (2020); S. W. Hawking, Nature 248, 30 (1974); F. C. Adams & G. Laughlin, Rev. Mod. Phys. 69, 337 (1997); B. P. Abbott et al., PRL 116, 061102 (2016); S. M. Carroll, Spacetime and Geometry (2004)."
    },
    expansion: {
    ...
    twotime: {
      ...
      sections: [
        ...
        ["A challenge to think with", () => Chrono.shows("exploratory") ? `${Chrono.expBanner()}<p>This project's own question: we describe time with questions written for space. What if time is its own <i>group</i> of dimensions — direction, texture, persistence — rather than one more axis? See the <a href="#dims">Dimension Map</a>. ${TG("HYPOTHESIS")}</p>` : `<p class="meta">Workbench mode adds this project's own exploratory take on this question.</p>`]
      ],
    ...
    tags: {
      ...
      sections: [
        ["The tags", `<dl class="tagdefs">
          ...
          <dt>${TG("HYPOTHESIS")}</dt><dd>This project's own ideas, or visitors' — not mainstream physics. Shown in Workbench mode, to be tested, not believed.</dd></dl>`],
  ...
  /* ---------------- the Concepts view ---------------- */
  const GROUPS = [["The four threads", ["clock", "now", "arrow", "dims"]], ["Core ideas", ["spacetime", "dilation", "entropy", "radiation", "expansion", "horizons", "quantum"]], ["Big questions, bold ideas", ["timetravel", "holography", "twotime"]], ["How this app works", ["scales", "tags", "hurdles"]]];
  ...
  const KEY = {
    ...
    story: ["arrow", "entropy", "expansion", "horizons"], hawking: ["horizons", "entropy", "holography", "radiation"], timeline: ["expansion", "arrow", "scales", "radiation"], energy: ["entropy", "radiation"], stars: ["entropy", "arrow"],
