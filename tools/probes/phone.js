...
window.addEventListener("load", () => setTimeout(async () => {
  ...
  const out = { routes: routes.length, overflow: [], header: [], small: [], blank: [], errors: __E };
  for (const r of routes) {
    ...
    if (c && c.width) { const d = c.getContext("2d").getImageData(0, 0, c.width, c.height).data, bg = [1, 3, 5].map(k => parseInt(Chrono.C.bg.substr(k, 2), 16)); let n = 0;
      for (let i = 0; i < d.length; i += 4 * 37) if (d[i + 3] > 0 && Math.abs(d[i] - bg[0]) + Math.abs(d[i + 1] - bg[1]) + Math.abs(d[i + 2] - bg[2]) > 30) n++;
      if (!n) out.blank.push(r); }
  }
